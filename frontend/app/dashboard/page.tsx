...
import { getDemo } from '../../components/api';
  ...
  return (
    <main className="container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div>
          <DemoRunButton onRun={runVerification} loading={loading} />
        </div>
      </div>
      ...
      <div className="card" style={{ borderLeft: '4px solid var(--accent)', background: 'rgba(45, 212, 255, 0.05)', marginBottom: 24 }}>
        <p style={{ margin: 0, fontSize: 14 }}>
          ⚠️ <strong>Synthetic Data Disclaimer:</strong> All included data, diagrams, and single-line files are synthetic and anonymized. This tool is designed for document consistency checks and validation, not real power-system or engineering design studies.
        </p>
      </div>

      {/* Grid: Upload Zone and Quick Stats */}
      <div className="grid grid2" style={{ marginBottom: 24, alignItems: 'stretch' }}>
        <UploadPanel onUploadSuccess={runVerification} />
        {data && <SummaryKpis kpis={data.kpis} />}
      </div>
      ...
      {loading ? (
        ...
        <div className="card critical" style={{ borderLeft: '4px solid var(--bad)', padding: 20 }}>
          <h3>Verification Engine Offline</h3>
          <p className="muted">{error}</p>
          <button onClick={runVerification} className="button" style={{ marginTop: 12 }}>Retry Connection</button>
        </div>
      ) : data ? (
        <div>
          {activeTab === 'overview' && (
          ...
          {activeTab === 'issues' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                <a href={`${process.env.NEXT_PUBLIC_API_BASE || 'http://localhost:8000'}/api/report/issues.csv`} className="button" style={{ padding: '8px 14px', fontSize: 13, textDecoration: 'none' }}>
                  📥 Export CSV
                </a>
              </div>
